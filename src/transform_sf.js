...
function transform_layer(layer, { parentBounds, parentHidden }) {
  ...
  /*
    Gets the screen dimensions of the target device's display.
    When in vertical or horizontal the screenBounds are in the form of {left:0, right: width, top:0, bottom:height} but
    during rotation there's also a transformation applied to the screen bounds (not the case in WM
    files) so to get the actual screen dimensions calculate {width: |right| - |left|, height: |bottom| - |top|}
   */
  if (layer.name.startsWith("Display Root#0") && layer.screenBounds) {
    bounds = {width: Math.abs(layer.screenBounds.right) - Math.abs(layer.screenBounds.left), height: Math.abs(layer.screenBounds.bottom) - Math.abs(layer.screenBounds.top)};
  }
